...
if ( ! class_exists( 'bookingpress_pro_staff_members' ) ) {
	class bookingpress_pro_staff_members Extends BookingPress_Core {
        ...
        public function get_bookingpress_service_data_group_with_category_for_staff($service_ids){
            ...
            $bookingpress_get_uncategorized_services = $wpdb->get_results($wpdb->prepare("SELECT * FROM $tbl_bookingpress_services WHERE bookingpress_category_id = %d AND bookingpress_service_id IN (".$service_ids.") ORDER BY bookingpress_service_id DESC", 0), ARRAY_A); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared --Reason: $tbl_bookingpress_services is table name
