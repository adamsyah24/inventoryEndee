...
if ( ! class_exists( 'bookingpress_pro_services' ) ) {
	class bookingpress_pro_services Extends BookingPress_Core {
		function __construct() {
			...
			add_filter( 'bookingpress_modify_service_time_with_buffer', array( $this, 'bookingpress_calculate_buffer_in_timeslot'), 10, 8 );
		...
		function bookingpress_calculate_buffer_in_timeslot( $timeslot_data, $slot_key, $selected_service_id, $booked_appointment_start_time, $booked_appointment_end_time, $current_time_start, $current_time_end, $selected_date = '' ){
			...
			if( 0 < $bookingpress_service_buffertime_before ){
				...
				if( empty( $selected_date ) ){
					$selected_date = !empty( $timeslot_data[ $slot_key ]['store_selected_date'] ) ? $timeslot_data[ $slot_key ]['store_selected_date'] : $timeslot_data[ $slot_key ]['selected_date'] ;
				}
				
				$buffer_start_time = date('Y-m-d H:i:s',strtotime( $selected_date . ' ' . $booked_appointment_start_time . '-' . $bookingpress_service_buffertime_before.' minutes' ) );
				$buffer_time_end = $selected_date . ' ' . $booked_appointment_start_time;

				$current_time_start = $selected_date .' ' . $current_time_start;
				$current_time_end = $selected_date .' ' .$current_time_end;
			...
			if( 0 < $bookingpress_service_buffertime_after ){
				...
				$buffer_time_start = $selected_date .' ' . $booked_appointment_end_time;
				$buffer_time_end = date('Y-m-d H:i:s', strtotime( $selected_date . ' ' . $booked_appointment_end_time . '+' . $bookingpress_service_buffertime_after.' minutes') );
