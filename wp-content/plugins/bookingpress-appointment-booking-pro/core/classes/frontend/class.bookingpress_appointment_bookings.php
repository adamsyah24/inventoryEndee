...
if ( ! class_exists( 'bookingpress_pro_appointment_bookings' )  && class_exists('BookingPress_Core')) {
	class bookingpress_pro_appointment_bookings Extends BookingPress_Core{
		...
		function bookingpress_password_validation_arr(){
			...
			$bpa_email = $_REQUEST['_email'];

			$check_password_field = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(bookingpress_field_type) FROM `{$tbl_bookingpress_form_fields}` WHERE bookingpress_field_type = %s AND bookingpress_field_is_hide = %d", 'password',0));
		...
		function bpa_password_field_validation_arr( $response ){
			...
			global $tbl_bookingpress_form_fields, $wpdb, $email_already_exists_flag, $bookingpress_front_vue_data_fields; 
				...
				if( !empty($check_password_field) && ( empty($bookingpress_front_vue_data_fields['appointment_step_form_data']['bpa_user_email_already_exists']) || $bookingpress_front_vue_data_fields['appointment_step_form_data']['bpa_user_email_already_exists'] == false ) ){
					...
					$bpa_username = $_REQUEST['_username'];
		...
		function bookingpress_before_selecting_booking_service_func($bookingpress_before_selecting_booking_service_data){
			...
			if( vm.is_staffmember_activated == 1 && vm.appointment_step_form_data.hide_staff_selection == "false" && vm.appointment_step_form_data.form_sequence[0] != "staff_selection" ){
				for( let index in vm.bookingpress_staffmembers_details ){
					...
					if( "undefined" != typeof currentValue.assigned_service_price_details[selected_service_id] && "public" == vm.bookingpress_staffmembers_details[index].staffmember_visibility ){
			...
			$bookingpress_vue_methods_data .= '
				...
				bookingpress_get_final_step_amount() {
					const vm = this;
						...
						/* vm.bookingpress_price_with_currency_symbol( total_payable_amount, true ); */
						/* if (typeof vm.appointment_step_form_data.cart_items == "undefined")  */
						/* { */
							 /* apply coupon */
		...
		function bookingpress_add_client_timezone( $bookingpress_disable_date_pre_xhr_data ){
			...
			if( "undefined" != typeof this.bookingpress_all_services_data[ selected_sid ].bookingpress_service_expiration_date && "" != this.bookingpress_all_services_data[ selected_sid ].bookingpress_service_expiration_date && null != this.bookingpress_all_services_data[ selected_sid ].bookingpress_service_expiration_date ) {
		...
		function bookingpress_frontend_add_appointment_data_variables( $bookingpress_front_vue_data_fields ) {
			...
			$all_external_fields = $wpdb->get_results( $wpdb->prepare( "SELECT bookingpress_field_meta_key,bookingpress_form_field_name FROM {$tbl_bookingpress_form_fields} WHERE bookingpress_field_type NOT IN ('2_col', '3_col', '4_col') AND bookingpress_field_is_default != %d", 1) ); // phpcs:ignore
			...
			if( is_user_logged_in() && !empty( $bpa_form_fields ) ){
				...
				$bpa_is_user_customer = $wpdb->get_row( $wpdb->prepare( "SELECT bookingpress_customer_id FROM {$tbl_bookingpress_customers} WHERE bookingpress_wpuser_id = %d ORDER BY bookingpress_customer_id DESC", $current_user_id ) );
				...
				if( !empty( $bpa_is_user_customer ) ){
					...
					foreach( $bpa_form_field_keys as $bpa_field_key ){
						if( !in_array( $bpa_field_key, $bpa_excluded_keys ) ){
							$is_customer_field = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( bookingpress_form_field_id ) FROM {$tbl_bookingpress_form_fields} WHERE bookingpress_field_meta_key = %s AND bookingpress_is_customer_field = %d", $bpa_field_key, 1) );
	
							if( 1 == $is_customer_field ){
								$meta_value = $wpdb->get_row( $wpdb->prepare( "SELECT bookingpress_customersmeta_value FROM {$tbl_bookingpress_customers_meta} WHERE bookingpress_customersmeta_key = %s AND bookingpress_customer_id = %d", $bpa_field_key, $current_user_customer_id ));
