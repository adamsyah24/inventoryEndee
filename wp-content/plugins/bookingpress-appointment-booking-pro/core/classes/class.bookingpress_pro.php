	...
	class BookingPressPro {
		...
		function bookingpress_booked_appointment_with_share_timeslot_where_clause_check_func($where_clause,$service_id = 0){
			...
			if($bookingpress_share_timeslot_between_services_type == 'service_category'){
				if(isset($_POST['appointment_data_obj']) && isset($_POST['appointment_data_obj']['selected_category'])){
					...
					$bookingpress_all_related_category_service = (isset($_POST['appointment_data_obj']['related_category_service']))?$_POST['appointment_data_obj']['related_category_service']:'';					
					...
					if(empty($bookingpress_related_services)){
						$selected_service = (isset($_POST['appointment_data_obj']['selected_service']))?$_POST['appointment_data_obj']['selected_service']:'';
					...
					if($service_id){
						...
						$bookingpress_related_services = array();
						$bookingpress_service_data = $wpdb->get_row($wpdb->prepare("SELECT bookingpress_category_id FROM {$tbl_bookingpress_services} WHERE bookingpress_service_id = %d ", $service_id), ARRAY_A);
						...
						if((!empty($bookingpress_category_id) || $bookingpress_selected_category == 0) && $bookingpress_selected_category != ''){
							$bookingpress_service_data = $wpdb->get_results($wpdb->prepare("SELECT bookingpress_service_id FROM {$tbl_bookingpress_services} WHERE bookingpress_category_id = %d ", $bookingpress_category_id), ARRAY_A);	
		...
		function bookingpress_install_pro_plugin_data(){
			...
			$bookingpress_default_notifications_message_arr        = array(
				'Appointment Rescheduled' => __('Dear %customer_full_name%,<br>You have successfully scheduled appointment.<br>Thank you for choosing us,<br>%company_name%','bookingpress-appointment-booking'),
				'Complete Payment URL'    => __('Hi<br/>Please complete your payment with following URL: <br/>%complete_payment_url%<br/>Thanks,<br/>%company_name%','bookingpress-appointment-booking'),
			...
			$bookingpress_default_notifications_arr2 = array(
				'Appointment Rescheduled' => __('Hi administrator,<br>You have one confirmed %service_name% appointment. The appointment is added to your schedule.<br>Thank you,<br>%company_name%','bookingpress-appointment-booking'),
				'Complete Payment URL'    => __('Hi administrator,<br/>Following payment URL is shared with customer. <br/>%complete_payment_url%<br/>Thank you,<br>%company_name%','bookingpress-appointment-booking'),
				'Refund Payment'    => __('Dear Administrator,<br /> The appointment %booking_id% has been canceled successfully, and the refund is initiated for the same from our end.<br />Thanks,<br />%company_name%','bookingpress-appointment-booking'),
			);
			foreach ( $bookingpress_default_notifications_name_arr as $bookingpress_default_notification_key => $bookingpress_default_notification_val ) {
			...
			$bookingpress_default_customer_notifications_arr2 = array(
				'Appointment Reminder' => __('Dear %customer_full_name%,<br> We would like to remind you that you have booked %service_name% on %appointment_date% at %appointment_time%. We are waiting for you at %company_address%.</br>Thank you for choosing our company,</br>%company_name%','bookingpress-appointment-booking'),
				'Appointment Follow Up' => __('Dear %customer_full_name%, <br> Thank you for choosing %company_name%. We hope you were satisfied with your %service_name%.<br>Thank you and we look forward to seeing you again soon,<br>%company_name%','bookingpress-appointment-booking'),
			);
