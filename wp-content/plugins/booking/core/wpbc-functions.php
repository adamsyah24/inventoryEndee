...
function wpbc_get_booking_params( $booking_id, $formdata, $booking_resource_id = 1 ) { 
	...
	//FixIn: 9.5.1.3
    $check_in_date_sql  = date_i18n( 'Y-m-d', strtotime( $sql_days_only_array[0] ) );
    $check_out_date_sql = date_i18n( 'Y-m-d', strtotime( $sql_days_only_array[ ( count( $sql_days_only_array ) - 1  ) ]  ) );
    $start_time_sql = date_i18n( 'H:i', mktime( $start_time[0], $start_time[1], $start_time[2] ) );
    $end_time_sql   = date_i18n( 'H:i', mktime( $end_time[0], $end_time[1], $end_time[2] ) );
    ////////////////////////////////////////////////////////////////////////////
        ...
        if ( $booking_form_show === '' ) {
            ...
            if ( ! class_exists('wpdev_bk_personal') ) {
                ...
                if ( class_exists('wpdev_bk_biz_m') ) {
	                ...
	                if ( false !== strpos( $formdata, 'wpbc_custom_booking_form' . $bktype . '^' ) ) {						//FixIn: 9.4.3.12

		                $custom_booking_form_name = substr( $formdata, strpos( $formdata, 'wpbc_custom_booking_form' . $bktype . '^' ) + strlen( 'wpbc_custom_booking_form' . $bktype . '^' ) );
...
//debuge($booking_form_show, $all_fields_array_without_types);die;
	...
	/**
	 * Get parsed booking data. - Get it as an array from 	'formatted booking form' string  data (usually from DB)
	 *
	 ...
	 * @return array
	 */
	function wpbc_get_parsed_booking_data_arr( $form_data, $resource_id = 1 ) {
		...
		foreach ( $fields_arr as $field ) {
			...
			$field_name = substr( $field_original_name, 0, - 1 * ( strlen( strval( $resource_id ) ) ) );
	...
	/**
	 *  Get encoded booking data string (Usually  for inserting into DB),  from  parsed booking data array
	 *
	 ...
	 * @return string
	 */
	function wpbc_encode_booking_data_to_string( $booking_data_arr, $resource_id = 1 ) {
...
//debuge($bk_otions);                        
     ...
     * @param string $subject	- string where to  search  shortcode
	 * @param int $pos					default 0
	 * @param string $pattern_to_search	default	'%\s*([^=]*)=[\'"]([^\'"]*)[\'"]\s*%'
	 *
	 * @return array|false
	 */
    function wpbc_get_params_of_shortcode_in_string( $shortcode, $subject , $pos = 0 , $pattern_to_search = '%\s*([^=]*)=[\'"]([^\'"]*)[\'"]\s*%' ) {   //FixIn: 7.0.1.8     7.0.1.52
	...
	/**
	 * Get shortcodes params and text for replacing these shortcodes
	 *
	 ...
	 */
	function wpbc_get_shortcodes_in_text( $content_text, $shortcode_arr = array( 'wpbc_dismiss' ) ) {                   //FixIn: 9.6.1.8
        ...
        if ( ( class_exists( 'WPBC_Dismiss' )) && ( ! wpbc_is_this_demo() ) ) {
            ...
            $is_panel_visible = $wpbc_Dismiss->render( 
                    array(
		                    ...
		                    'hint'  => sprintf( __( 'Dismiss', 'booking' ) ),
            ...
            <div class="welcome-panel-column">
                ...
                <ul>
                    <li><div class="welcome-icon"><?php
                            printf( __( 'Insert booking form %sshortcode%s into your %sPost%s or %sPage%s','booking')
                    ...
                    <li><div class="welcome-icon"><?php
                            printf( __( 'Check %show todo%s that and what %sshortcodes%s are available.','booking'),
                                '<a href="https://wpbookingcalendar.com/faq/insert-booking-calendar-into-page/" target="_blank">',
                                '</a>',
