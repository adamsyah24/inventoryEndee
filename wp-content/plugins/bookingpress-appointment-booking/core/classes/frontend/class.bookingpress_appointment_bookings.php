...
if (! class_exists('bookingpress_appointment_bookings')  && class_exists('BookingPress_Core')) {
    class bookingpress_appointment_bookings Extends BookingPress_Core
    {
            ...
            foreach( $period as $dt ){
                ...
                if( !in_array( $date_t, $daysoff_dates ) ){
                    ...
                    if( false == $is_time_slot_available ){
                        ...
                        $extra_disabled_dates[] = date('Y-m-d H:i:s', strtotime( $date_t) );
                    }
                }
            }
         ...
         * @return void
         */
        function bookingpress_get_disable_date_func( $bpa_selected_date = '', $consider_selected_date = false, $counter = 1, $bpa_total_booked_appointment = array(), $single_disable_date = array()) {
            ...
            if( !empty( $front_timings ) && !$is_custom_duration ){
                ...
                if( true == $is_front_timings_empty  ){
                    ...
                    $this->bookingpress_get_disable_date_func( $response['next_available_date'], true, $counter++, $bookingpress_total_appointment, $single_disable_date );
        ...
        function bookingpress_before_book_appointment_func()
        {
            ...
            $bookingpress_form_token = !empty( $_REQUEST['appointment_data']['bookingpress_form_token'] ) ? sanitize_text_field( $_REQUEST['appointment_data']['bookingpress_form_token'] ) : sanitize_text_field( $_REQUEST['appointment_data']['bookingpress_uniq_id'] ) ;
            ...
            if(!empty($bookingpress_uniq_id)){
                if(!empty($_COOKIE['bookingpress_cart_id'])) {
                    ...
                    if(!empty($appointment_id)){
                        $bookingpress_appointment_details = $wpdb->get_row($wpdb->prepare("SELECT bookingpress_booking_id FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_order_id = %d", $appointment_id), ARRAY_A);
	    ...
	    $bookingpress_site_current_lang_moment_locale = get_locale();
            ...
            if( null != document.getElementById('$bookingpress_vue_root_element_id_without_hash').innerHTML ){
