...
if ( ! function_exists( 'ovarw_taxi_get_distance_text' ) ) {
	function ovarw_taxi_get_distance_text( $distance ) {
        if ( $distance ) {
            $distance = apply_filters( 'ovabrw_ft_distance_text', round( $distance / 1000, 2 ) );
        } else {
            $distance = 0;
        }

        return sprintf( esc_html__( '%s km', 'ova-brw' ), $distance );
	}
}
...
if ( ! function_exists( 'ovabrw_wc_price' ) ) {
	function ovabrw_wc_price( $price ) {
		$html = wc_price( $price );

		return apply_filters( 'ovabrw-ft-wc-price', $html, $price );
	}
}
